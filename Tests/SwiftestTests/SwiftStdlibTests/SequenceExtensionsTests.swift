...
//  Swiftest
//
//  Created by Brian Strobach on 04/04/2018.
...
final class SequenceExtensionsTests: XCTestCase {
    ...
    func testContains() {
        XCTAssert([Int]().contains(all: []))
        XCTAssertFalse([Int]().contains(all: [1, 2]))
        XCTAssert([1, 2, 3].contains(all: [1, 2]))
        XCTAssert([1, 2, 3].contains(all: [2, 3]))
        XCTAssert([1, 2, 3].contains(all: [1, 3]))
        XCTAssertFalse([1, 2, 3].contains(all: [4, 5]))
    }
