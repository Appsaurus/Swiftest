...
public extension Sequence {
    ...
    // Alias for `filter(_ isIncluded: (Element) throws -> Bool) rethrows -> [Element]` to be more semantic
    public func find(where condition: (Element) throws -> Bool) rethrows -> [Element] {
        return try filter { return try condition($0) }
    }
}
